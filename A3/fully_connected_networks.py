...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        dh, grads["W2"], grads["b2"] = Linear.backward(dx, cache2)
        W2 = self.params["W2"]
...
class FullyConnectedNet(object):
    ...
    def __init__(
        ...
        for i, hs in enumerate(hidden_dims):
              self.params["W" + str(i + 1)] = weight_scale * torch.randn(
                  prev_dim, hs, dtype=dtype, device=device
              )
              self.params["b" + str(i + 1)] = torch.randn(
                  hs, dtype=dtype, device=device
              )
              prev_dim = hs
        self.params["W" + str(self.num_layers)] = weight_scale * torch.randn(
            hidden_dims[len(hidden_dims) - 1], num_classes, dtype=dtype, device=device
        )
        self.params["b" + str(self.num_layers)] = torch.randn(
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            ...
            caches.append(cache)
            if self.use_dropout:
              out, drop_cache = Dropout.forward(
                out, self.dropout_param
              )
              drop_caches.append(drop_cache)
        out, cache = Linear.forward(
            out,
        ...
        d, grads["W" + str(self.num_layers)], grads["b" + str(self.num_layers)] = (
            Linear.backward(d, caches[self.num_layers - 1])
        )
        W = self.params["W" + str(self.num_layers)]
        grads["W" + str(self.num_layers)] += self.reg * W
        loss += (self.reg / 2) * torch.sum(W * W)

        for i in range(self.num_layers - 2, -1, -1):
            if self.use_dropout:
              d = Dropout.backward(d, drop_caches[i])
            d, grads["W" + str(i + 1)], grads["b" + str(i + 1)] = Linear_ReLU.backward(
                d, caches[i]
            )
...
def rmsprop(w, dw, config=None):
    ...
    ep = config["epsilon"]
    ca = config["cache"]
    new_ca = dr * ca + (1 - dr) * dw**2
    next_w = w - lr * dw / (new_ca.sqrt() + ep)
    config["cache"] = new_ca
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
