...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        for img_idx in range(N):
            for ftr_idx in range(F):
                img = padded_x[img_idx]
                ftr = w[ftr_idx]
                for i in range(out_W):
                    for j in range(out_H):
                        ...
                        out[img_idx, ftr_idx, j, i] = torch.sum(slice * ftr)
                out[img_idx, ftr_idx, :, :] += b[ftr_idx]
        #####################################################################
        #                          END OF YOUR CODE                         #
        #####################################################################
        cache = (x, w, b, conv_param)
            ...
            for ftr_idx in range(F):
                for i in range(out_W):
                    for j in range(out_H):
                        ...
                        dw[ftr_idx] += slice * dout[img_idx, ftr_idx, j, i]
                        ...
                        dpad_x[
                            ...
                            w[ftr_idx] * dout[img_idx, ftr_idx, j, i]
                        )
                ...
                for h in range(pool_h):
                    for w in range(pool_w):
                        ...
                        dx[
                            ...
                            h * stride : h * stride + pool_h,
                            w * stride : w * stride + pool_w,
                        ] = (slice == torch.max(slice)) * dout[n, c, h, w]
        ####################################################################
        #                          END OF YOUR CODE                        #
        ####################################################################
        return dx
...
class ThreeLayerConvNet(object):
    ...
    def __init__(
        ...
        self.params["W2"] = weight_scale * torch.randn(
            num_filters * (input_dims[1] // 2) * (input_dims[2] // 2),
            hidden_dim,
            device=device,
            dtype=dtype,
        )
        self.params["b2"] = torch.zeros(hidden_dim, device=device, dtype=dtype)
        self.params["W3"] = weight_scale * torch.zeros(
            hidden_dim, num_classes, device=device, dtype=dtype
        )
...
class DeepConvNet(object):
    ...
    def __init__(
        ...
        conv_param = {"stride": 1, "pad": (filter_size - 1) // 2}
        pool_param = {"pool_height": 2, "pool_width": 2, "stride": 2}
        chan, H_out, W_out = input_dims
        for i, nf in enumerate(num_filters):
            H_out = (
                1
                + (H_out + 2 * conv_param["pad"] - filter_size) // conv_param["stride"]
            )
            W_out = (
                1
                + (W_out + 2 * conv_param["pad"] - filter_size) // conv_param["stride"]
            )
            ...
            if self.batchnorm:
                self.params["gamma" + str(i + 1)] = 0.01 * torch.randn(
                    nf, device=device, dtype=dtype
                )
                self.params["beta" + str(i + 1)] =  0.01 * torch.randn(
                    nf, device=device, dtype=dtype
                )

            if weight_scale == "kaiming":
                self.params["W" + str(i + 1)] = kaiming_initializer(
                    nf, chan, filter_size, relu=True, device=device, dtype=dtype
                )
            else:
                self.params["W" + str(i + 1)] = weight_scale * torch.randn(
                    nf, chan, filter_size, filter_size, device=device, dtype=dtype
                )
            self.params["b" + str(i + 1)] = torch.zeros(nf, device=device, dtype=dtype)
            chan = nf

        if weight_scale == "kaiming":
            self.params["W" + str(self.num_layers)] = kaiming_initializer(
                chan * H_out * W_out, num_classes, relu=True, device=device, dtype=dtype
            )
        else:
    ...
    def loss(self, X, y=None):
        ...
        for i in range(self.num_layers - 1):
            if i in self.max_pools:
                if self.batchnorm:
                    out, caches[i + 1] = Conv_BatchNorm_ReLU_Pool.forward(
                        out,
                    ...
                    out, caches[i + 1] = Conv_ReLU_Pool.forward(
                        out,
                        self.params["W" + str(i + 1)],
                        self.params["b" + str(i + 1)],
                        conv_param,
                ...
                if self.batchnorm:
                    out, caches[i + 1] = Conv_BatchNorm_ReLU.forward(
                        out,
                        self.params["W" + str(i + 1)],
                        self.params["b" + str(i + 1)],
                        self.params["gamma" + str(i + 1)],
                    ...
                    out, caches[i + 1] = Conv_ReLU.forward(
                        out,
                        self.params["W" + str(i + 1)],
                        self.params["b" + str(i + 1)],
                        conv_param,
                    )
        out, caches[self.num_layers] = Linear.forward(
            out,
        ...
        dout, grads["W" + str(self.num_layers)], grads["b" + str(self.num_layers)] = (
            Linear.backward(dout, caches[self.num_layers])
        )
        loss += self.reg * torch.sum(self.params["W" + str(self.num_layers)] ** 2)
        grads["W" + str(self.num_layers)] += (
            2 * self.reg * self.params["W" + str(self.num_layers)]
        )

        # for the rest macro layers
        for i in range(self.num_layers - 2, -1, -1):
            if i in self.max_pools:
                if self.batchnorm:
                    ...
                    grads["gamma" + str(i + 1)] = (
                        dgamma + 2 * self.reg * self.params["gamma" + str(i + 1)]
                    )
                    loss += self.reg * torch.sum(self.params["W" + str(i + 1)] ** 2) + self.reg * torch.sum(self.params["gamma" + str(i + 1)] ** 2)
                else:
                    dout, grads["W" + str(i + 1)], grads["b" + str(i + 1)] = (
                        Conv_ReLU_Pool.backward(dout, caches[i + 1])
                    )
                    grads["W" + str(i + 1)] += (
                        2 * self.reg * self.params["W" + str(i + 1)]
                    )
...
def kaiming_initializer(
    ...
    if K is None:
        ...
        fan_in = Din * K * K
        weight_scale = (gain / fan_in) ** (1 / 2)
        weight = weight_scale * torch.randn(Din, Dout, K, K, device=device, dtype=dtype)
        ###################################################################
        #                         END OF YOUR CODE                        #
        ###################################################################
    return weight
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == "train":
            ...
            sample_var = (centered_x**2).mean(dim=0)
            running_var = momentum * running_var + (1 - momentum) * sample_var
            sqrt_var = torch.sqrt(sample_var + eps)
            norm_x = centered_x / sqrt_var
            out = gamma * norm_x + beta
            cache = (norm_x, gamma, centered_x, sqrt_var, sample_var, eps)
            ################################################################
            #                           END OF YOUR CODE                   #
            ################################################################
        elif mode == "test":
            ...
            ################################################################
            # Replace "pass" statement with your code
            norm_x = (x - running_mean) / torch.sqrt(running_var + eps)
    ...
    def backward(dout, cache):
        ...
        #####################################################################
        # Replace "pass" statement with your code
        norm_x, gamma, centered_x, sqrt_var, sample_var, eps = cache
        N, D = dout.shape
        dbeta = dout.sum(dim=0)
        dgamma = dout * norm_x
        dgamma = dgamma.sum(dim=0)
        dnorm_x = dout * gamma
        dcentered_x1 = dnorm_x / sqrt_var
        dinv_sqrt_var = dnorm_x * centered_x
        dinv_sqrt_var = dinv_sqrt_var.sum(dim=0)
        dsqrt_var = -1 * dinv_sqrt_var / (sqrt_var**2)
        dsample_var = 0.5 * ((sample_var + eps) ** (-1 / 2)) * dsqrt_var
        dsq = 1.0 / N * torch.ones((N, D), device=dout.device) * dsample_var
        dcentered_x2 = 2 * centered_x * dsq
        dcentered_x = dcentered_x1 + dcentered_x2
        dx1 = dcentered_x
        dmean = -1 * torch.sum(dx1, dim=0)
        dx2 = 1.0 / N * torch.ones((N, D), device=dout.device) * dmean
        dx = dx1 + dx2
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
...
class SpatialBatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = x.shape
        new_x = x.permute(1, 0, 2, 3).reshape(C, -1).t()
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        N, C, H, W = dout.shape
        new_dout = dout.permute(1, 0, 2, 3).reshape(C, -1).t()
