...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for t in range(num_train):
        train_vec = x_train[t].view(-1)
        for e in range(num_test):
            test_vec = x_test[e].view(-1)
            dist = (train_vec - test_vec).square().sum()
            dists[t, e] = dist
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    test_vecs = x_test.view(num_test, -1) # torchSize([250, 3072])
    for t in range(num_train):
        train_vec = x_train[t].view(-1)
        dists[t, :] = (test_vecs - train_vec).square().sum(dim=1).t()
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def knn_cross_validate(
    ...
    for k in k_choices:
        ...
        for i in range(num_folds):
            x_train_set = torch.cat(x_train_folds[0:i] + x_train_folds[i+1:num_folds], dim=0)
            y_train_set = torch.cat(y_train_folds[0:i] + y_train_folds[i+1:num_folds], dim=0)
            x_valid_set = x_train_folds[i]
            y_valid_set = y_train_folds[i]
            classifier = KnnClassifier(x_train_set, y_train_set)
            k_to_accuracies[k].append(classifier.check_accuracy(x_valid_set, y_valid_set, k))
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
