...
def nn_forward_backward(
    ...
    softmax_scores = scores.exp() / scores.exp().sum(dim=1, keepdim=True)
    loss = -softmax_scores[torch.arange(N), y].log().sum()
    loss /= N
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    softmax_scores[torch.arange(N), y] -= 1
    dW2 = h1.t() @ softmax_scores
    dh1 = softmax_scores @ W2.t()
    dh1[h1 == 0] = 0
    dW1 = X.t() @ dh1
    db2 = softmax_scores.sum(dim=0)
    db1 = dh1.sum(dim=0)
...
def find_best_net(data_dict: Dict[str, torch.Tensor], get_param_set_fn: Callable):
    ...
    # Replace "pass" statement with your code
    i = 1
    hparams = get_param_set_fn()
    lrs = hparams[0]
    hsizes = hparams[1]
    regs = hparams[2]
    lrdecays = hparams[3]
    num_models = len(lrs) * len(hsizes) * len(regs) * len(lrdecays)
    for lr in lrs:
        for hsize in hsizes:
            for reg in regs:
                for lrdecay in lrdecays:
                    print(
                        "Training %d / %d model"
                        % (i, num_models)
                    )
                    i += 1
                    curr_net = TwoLayerNet(
                        3 * 32 * 32, hsize, 10, dtype=data_dict['X_train'].dtype, device=data_dict['X_train'].device
                    )
                    curr_stat = curr_net.train(
                        data_dict['X_train'],
                        data_dict['y_train'],
                        data_dict['X_val'],
                        data_dict['y_val'],
                        learning_rate=lr,
                        learning_rate_decay=lrdecay,
                        reg=reg,
                        num_iters=3000,
                        batch_size=1000,
                        verbose=False,
                    ...
                    y_pred_val_acc = (
                        (curr_net.predict(data_dict['X_val']) == data_dict['y_val'])
                        .float()
                        .mean()
                        .item()
                    )
                    if y_pred_val_acc > best_val_acc:
                        best_val_acc = y_pred_val_acc
                        best_net = curr_net
                        best_stat = curr_stat
    #############################################################################
    #                               END OF YOUR CODE                            #
