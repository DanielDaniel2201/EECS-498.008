...
def svm_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i]) # shape (C,), a column vector with scores of this image X[i]
        correct_class_score = scores[y[i]] # the score (a scalar) for the correct class in scores
        for j in range(num_classes):
            if j == y[i]:
                continue # ignore correct class when computing SVM loss
            margin = scores[j] - correct_class_score + 1  # note delta = 1
            if margin > 0:
                loss += margin # compute the SVM loss
                #######################################################################
                # TODO:                                                               #
                ...
                # pass
                dW[:, j] += X[i] / num_train # update the "incorrect" classifier
                dW[:, y[i]] += -X[i] / num_train # update the "correct" classifier by scaling xi
                #######################################################################
                #                       END OF YOUR CODE                              #
                #######################################################################
...
def svm_loss_vectorized(
    ...
    num_train = X.shape[0]
    scores = X @ W # size (N, C)
    col = torch.arange(X.shape[0]) # size N
    correct_class_scores = scores[col, y].reshape(num_train, 1) # size(N, 1)
    ...
    #############################################################################
    # Replace "pass" statement with your code
    margins[margins > 0] = 1 # a matrix of 0 and 1
    margins[col, y] = torch.sum(margins, dim=1) * -1 # update the scalars for correct class classifiers
    dW = (X.t() @ margins) / num_train # D * C
    dW += 2 * W * reg
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
...
def test_one_param_set(
    ...
    train_acc = (y_train == train_pred).sum() / num_train
    val_pred = cls.predict(X_val)
    val_acc = (y_val == val_pred).sum() / num_val
    ############################################################################
    #                            END OF YOUR CODE                              #
    ############################################################################
...
def softmax_loss_naive(
    ...
    for i in range(num_train):
        scores = W.t().mv(X[i])
        scores -= scores.max() # normalize for numeric statbility
        softmax_scores = scores.exp() / scores.exp().sum()
        loss += -softmax_scores[y[i]].log() / num_train
        for j in range(num_class):
          dW[:, j] += X[i] * softmax_scores[j] / num_train
...
def softmax_loss_vectorized(
    ...
    num_train = X.shape[0]
    raw_scores = X @ W
    max_scores = raw_scores.amax(dim=1, keepdim=True)
    raw_scores -= max_scores
    softmax_scores = raw_scores.exp() / raw_scores.exp().sum(dim=1, keepdim=True)
    loss = -softmax_scores[torch.arange(num_train), y].log().sum()
